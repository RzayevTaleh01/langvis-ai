...
export class TutorWalker {
  ...
  constructor(board, node, face) {
    ...
    board.addEventListener("scroll", () => { if (!this.atHome) this.reflow(); }, { passive: true });
    ...
    if ("ResizeObserver" in window) {
      new ResizeObserver(() => { if (!this.atHome) this.reflow(); }).observe(board.firstElementChild || board);
