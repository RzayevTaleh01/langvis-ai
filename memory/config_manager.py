...
def set_gemini_keys(main: str, extras: list[str]) -> None:
    ensure_config_dir()
    data = load_api_keys()
    data["gemini_api_key"] = str(main or "").strip()
    data["gemini_extra_keys"] = [k for k in dict.fromkeys(str(x).strip() for x in extras)
                                 if len(k) > 15 and k != data["gemini_api_key"]]
...
def save_assistant_config(assistant_name: str, user_name: str) -> None:
    ...
    if CONFIG_FILE.exists():
        try:
            ...
            data = {}
    data["assistant_name"] = assistant_name.strip() or "LangVis"
