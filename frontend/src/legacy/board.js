...
export class Board {
  constructor({ walker, audio, send }) {
    ...
    this.onUndo = () => this.send({ type: "undo" });
    $("undo").addEventListener("click", this.onUndo);
  }

  // The page closes: the timer and the listener go with it.
  destroy() {
    clearInterval(this.timer);
    $("undo")?.removeEventListener("click", this.onUndo);
  }
